...
export default function CompanyDetails(props) {
    ...
    // redux variables
    const currentDataCompanies = useSelector(state => state.currentDataCompanies);

    return(
        <>
            <EditCompany  show={showEdit} handleShow={handleShowEdit}/>
            <div className={`details-main-body bg-whitex ${props.show}`}>
                <DashHeaderDetails title={currentDataCompanies.company_name} handleShow={props.handleShow}/>
            
                <div className='my-3 text-lg pl-3 font-bold txt-headr sm:flex lg:hidden relative'>
                    {currentDataCompanies.company_name}<br/>
                     {/* <span className={`sm:block hidden flt-id xwarn`}>Pending</span> */}
                    {/* <span className={`sm:hidden block-inline xwarn`}>Pending</span> */}
                </div>
