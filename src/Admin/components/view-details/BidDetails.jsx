...
export default function BidDetails(props) {
    ...
    var status_cn;
    var pro_status;
    if(props.status !== undefined){pro_status = currentDataBids.statusx.toLowerCase();}
    ...
    return(
        <>
            ...
            <div className={`details-main-body bg-whitex ${props.show}`}>
                <DashHeaderDetails title={currentDataBids.name} handleShow={props.handleShow} status={currentDataBids.statusx}/>
                <div className='my-3 text-lg pl-3 font-bold txt-headr md:flex lg:hidden relative'>
                    {currentDataBids.name}<br/> <span className={`md:block hidden flt-id xwarn`}>Pending</span>
                    <span className={`md:hidden block-inline xwarn ${status_cn} uppercase`}>{pro_status}</span>
                </div>
