  ...
  const dispatch = useDispatch();
    return (
        ...
        <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-cus-1 gap-y-cus-1">
            {slice.map(e=>{
              var status_cn;
              if(e.status.toLowerCase() === 'active'){status_cn = 'ml-auto capitalize bg-green-100 text-green-500 rounded-lg text-xs p-1 px-2'}
              else if(e.status.toLowerCase() === 'rejected'){status_cn = 'ml-auto capitalize bg-red-100 text-red-500 rounded-lg text-xs p-1 px-2'}
